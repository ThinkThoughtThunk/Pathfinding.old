...
function djikstra(graph) {
	...
	const lowestCostVertexFrom = (vertices) => {
		...
		for (let i in vertices) {
			...
			else if (shortestDistanceTo(vertex) < shortestDistanceTo(minimum)) {
				minimum = vertex;
			}
		}
		return minimum;
...
function createVertices(width, height) {
	...
	for (let row = 0; row < height; row++) {
		for (let column = 0; column < width; column++) {
			...
			let vertex = new Vertex(id, { x: column, y: row });
			vertices[id] = vertex;
		}
	}
	return vertices;
...
function neighborsOf(vertex) {
	...
	function surroundings(vertex) {
		...
		return [
			// up:
			{ cell: getCell(x, y - 1), diagonal: false },
			// upRight:
			{ cell: getCell(x + 1, y - 1), diagonal: false },
			// right:
			{ cell: getCell(x + 1, y), diagonal: false },
			// downRight:
			{ cell: getCell(x + 1, y + 1), diagonal: true },
			// down:
			{ cell: getCell(x, y + 1), diagonal: false },
			// downLeft:
			{ cell: getCell(x - 1, y + 1), diagonal: true },
